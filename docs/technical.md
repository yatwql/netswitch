...
2. 判类型：存在 `/sys/class/net/<if>/wireless` → 无线，否则有线（TUI 高亮区分）。
...
- **启动即拉取 + 自动刷新**：进入即拉取实时网卡信息（数量 1~N、有线/无线、连接状态、IP、metric、默认路由）；界面每 ~1.5s 自动刷新，`f` 或 `F5` 手动刷新；底部状态栏显示“最后刷新 时间”。因此开启网卡后 DHCP 获取 IP、链路变化会自动显示。
...
- **规则区**：列出配置中的每条规则，显示**生效网卡**与是否**已应用**；`g` 切换选中规则、`e` 选生效网卡、`p` 应用、`c` 撤销。
